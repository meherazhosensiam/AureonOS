...
class ApplicationListSettingsWidget extends Adw.PreferencesGroup {
    static {
        GObject.registerClass(this);
    ...
    constructor(settings) {
        super({
            title: _('Super + Tab switch applications'),
        });
        this._settings = settings;
        this._rules = new RulesList(settings);
...
export default class MyExtensionPreferences extends ExtensionPreferences {
    ...
    fillPreferencesWindow(window) {
        ...
        // all desktops
        const allDesktopsSwitch = new Gtk.Switch({});
        allDesktopsSwitch.set_valign(Gtk.Align.CENTER);
        const allDesktopsRow = new Adw.ActionRow({
            title: _('Show from all workspaces'),
        });
        allDesktopsRow.add_suffix(allDesktopsSwitch);
        group.add(allDesktopsRow);

        // minimised
        const applicationListWidget = new ApplicationListSettingsWidget(this.getSettings());
        group.add(applicationListWidget);

        page.add(group);
        window.add(page);
        window.set_default_size(applicationListWidget.width, applicationListWidget.height);
        window._settings.bind('show-minimized', minimizedSwitch, 'active',
            Gio.SettingsBindFlags.DEFAULT);
        window._settings.bind('all-desktops', allDesktopsSwitch, 'active',
            Gio.SettingsBindFlags.DEFAULT);
        applicationListWidget.show();
